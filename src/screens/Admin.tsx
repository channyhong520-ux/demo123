...
export default function Admin({ onBack }: { onBack: () => void }) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMsg({ ok: true, text: "✅ បានបន្ថែមទំនិញរួចរាល់" });
    } catch (err: unknown) {
      console.error(err);
      const detail =
  ...
  return (
    <div className="flex flex-col h-full bg-[#f2f7f5]">
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar px-4 pt-3 pb-8">
        {/* Add product form */}
        <form onSubmit={submit} className="bg-white rounded-2xl card-shadow p-4">
          <div className="text-[#148c78] font-semibold text-sm mb-3">➕ បន្ថែមទំនិញថ្មី</div>
